...
# time
d[, time := as.factor(time)]
d[, time := relevel(time, "Midnight - 8 AM")]
#d[ , .N, by = .(time, gender)]
...
plot8 <- ggplot(d, aes (x=score_improved))+ geom_histogram() + ggtitle('Distribution of score_improved') + geom_vline(xintercept = d[treatment == 1, mean(score_improved)], col = 'turquoise3', lwd = 1) +
  geom_vline(xintercept = d[treatment == 0, mean(score_improved)], col = 'salmon', lwd = 1)

grid.arrange(plot5, plot6, plot7, plot8, ncol = 1, top = 'Histograms of Scores')
...
m1 <- reg(d[, lm(as.numeric(treatment) ~  age + music_training + gender +  time )],
...
```{r}
covs = subset(d, select = c(age, music_training, time, gender))
...
```{r warning=FALSE}
m0 <- reg(d[,lm(score_improved ~ treatment + music_training + time + gender + gender * treatment)])

m1 <- reg(d[age == '5 - 21 years',lm(score_improved ~  treatment +   music_training + time + gender + gender * treatment)], ptest = FALSE)
m2 <- reg(d[age == '22 - 55 years' ,lm(score_improved ~  treatment + music_training + time + gender + gender * treatment)], ptest = FALSE)
m3 <- reg(d[age == 'Older than 55 years' ,lm(score_improved ~  treatment + music_training + time + gender + gender * treatment)], ptest = FALSE)
stargazer( m0$model, m1$model, m2$model, m3$model,
 type = 'text',
...
```{r warning=FALSE}
m0 <- reg(d[,lm(score_improved ~ music_training + time + treatment * time + gender + gender * treatment)])

m1 <- reg(d[age == '5 - 21 years',lm(score_improved ~   music_training + time + treatment * time + gender + gender * treatment)], ptest = FALSE)
m2 <- reg(d[age == '22 - 55 years' ,lm(score_improved ~  music_training + time + treatment * time + gender + gender * treatment)], ptest = FALSE)
m3 <- reg(d[age == 'Older than 55 years' ,lm(score_improved ~  music_training + time + treatment * time + gender + gender * treatment)], ptest = FALSE)
stargazer( m0$model, m1$model, m2$model, m3$model,
 type = 'text',
...
```{r warning=FALSE}
m0 <- reg(d[,lm(score_difference ~  treatment +  music_training + time + gender + gender * treatment)])

m1 <- reg(d[age == '5 - 21 years',lm(score_difference ~  treatment +  music_training + time + gender + gender * treatment)], ptest = FALSE)
m2 <- reg(d[age == '22 - 55 years' ,lm(score_difference ~  treatment +  music_training + time + gender + gender * treatment )], ptest = FALSE)
m3 <- reg(d[age == 'Older than 55 years' ,lm(score_difference ~  treatment +  music_training + time + gender + gender * treatment )], ptest = FALSE)
stargazer( m0$model, m1$model, m2$model, m3$model,
 type = 'text',
 title = 'Long Model - Score Difference',
 se = list(sqrt(diag(vcovHC(m0$model, type="HC1"))),
           sqrt(diag(vcovHC(m1$model, type="HC1"))), 
           sqrt(diag(vcovHC(m2$model, type="HC1"))), 
           sqrt(diag(vcovHC(m3$model, type="HC1")))),
...
```{r warning=FALSE}
m0 <- reg(d[,lm(score_difference ~  music_training + time + treatment * time + gender + gender * treatment)])

m1 <- reg(d[age == '5 - 21 years',lm(score_difference ~  music_training + time + treatment * time + gender + gender * treatment)], ptest = FALSE)
m2 <- reg(d[age == '22 - 55 years' ,lm(score_difference ~  music_training + time + treatment * time + gender + gender * treatment )], ptest = FALSE)
m3 <- reg(d[age == 'Older than 55 years' ,lm(score_difference ~  music_training + time + treatment * time + gender + gender * treatment )], ptest = FALSE)
stargazer( m0$model, m1$model, m2$model, m3$model,
 type = 'text',
 title = 'Score Difference - treatment HTES with time of day',
 se = list(sqrt(diag(vcovHC(m0$model, type="HC1"))),
           sqrt(diag(vcovHC(m1$model, type="HC1"))), 
           sqrt(diag(vcovHC(m2$model, type="HC1"))), 
           sqrt(diag(vcovHC(m3$model, type="HC1")))),
...
```{r}
contingency_table <- table(d$age, d$treatment)
contingency_table

chisq.test(contingency_table[2:3,])
```

### Music and No-Music (T -Test)
``` {r}
s1 <- reg(d[, lm(score_improved ~ music_training)], ptest = TRUE)
s1
```